...
// what part to print
part = 1; // [1:print just the box,2:print just the lid,3:print the box and the lid] 
...
topfan					= false;	// [true:YES with top fan,false:NO without top fan]
...
resetbutton				= false;	//[true:YES with reset button,false:NO without reset button]	
	...
	difference() {
		union() {
			cnccube(x=ramps_x+ramps_xtra_left+ramps_xtra_right,
                    ...
                    z=ramps_z, r=ticknesss_corners, w=thickness_bottom);
		...
		*if(knightmount) {
		...
		if(backfan) {
			translate([ramps_x/2+ramps_xtra_left,ramps_calc_y+ramps_xtra_front+ramps_xtra_back+(ticknesss_corners)*2+1,ramps_z/2]) rotate([90,0,0])
        ...
        translate([ramps_xtra_left,ramps_xtra_front,ramps_z_spacing+thickness_bottom]) {
			...
			translate([8.26,-ticknesss_corners*2-1,0.5])
				cube([14.11,thickness_front+2,ramps_z-ramps_z_spacing-thickness_bottom-bridgetop]);
			translate([1.35,-ticknesss_corners*2-1,14])
				cube([ramps_x-1.35*2,thickness_front+2,ramps_z-14-ramps_z_spacing-bridgetop]);
			if(resetbutton) {
				translate([ramps_x+ramps_xtra_front+(ticknesss_corners)*2,35.44,18.07]) rotate([0,-90,0]) fncylinder(r=5,h=(ticknesss_corners)*2,enlarge=1);
			}
			if (backopening) {
				translate([1.35,ramps_xtra_front+ramps_calc_y+ramps_xtra_back,backopening_bottomwall])
					cube([ramps_x-1.35*2,ticknesss_corners*2+2,ramps_z-backopening_bottomwall-ramps_z_spacing-bridgetop]);
			}
		}
        ...
        for (y=[10:40:90]) translate([30,y,30]) rotate([0,90,0]) 
            cylinder(d=6,h=100);
	}
}
