...
module Zroller2D(holes=1) {
    difference() {
        ...
        if (holes) {
            // Axle space
            square([3/8*inch+1,200],center=true);
            ...
            for (topbot=[-1,+1]) scale([1,topbot,1])
                translate([0,ZrollerZ/2])
                {
                    ...
                    // Space above bearing
                    square([bearingOD(bearing)-5,2*bearingZ(bearing)+2],center=true);
                }        
        }
...
module Zroller_holder() {
    ...
    clear=1.5; // clearance around moving roller
        ...
        ZrollerC() difference() {
            ...
            for (topbot=[-1,+1]) scale([1,1,topbot])
                translate([0,0,ZrollerZ/2])
                    cylinder(d1=Zroller_bolt+2,d2=Zroller_bolt+2+2*taper,h=taper);
        }
